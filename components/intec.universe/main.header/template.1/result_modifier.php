...
if ($arResult['CONTACTS']['ADVANCED']) {
    $bShow = false;
    $arShow = [
        'ADDRESS' => $arParams['CONTACTS_ADDRESS_SHOW'] === 'Y' && !empty($arCodes['CONTACTS']['ADDRESS']),
        'EMAIL' => $arParams['CONTACTS_EMAIL_SHOW'] === 'Y' && !empty($arCodes['CONTACTS']['EMAIL']),
        'PHONE' => !empty($arCodes['CONTACTS']['PHONE']),
        'SCHEDULE' => $arParams['CONTACTS_SCHEDULE_SHOW'] === 'Y' && !empty($arCodes['CONTACTS']['SCHEDULE'])
    ];
    ...
    if (!empty($arParams['CONTACTS_IBLOCK_ID']) && $bShow) {
        ...
        $arContacts = $arContacts->asArray(function ($iId, $arContact) use (&$arShow, &$arCodes) {
            ...
            foreach ($arShow as $sKey => $bShow) {
                ...
                if ($bShow) {
                    $sValue = ArrayHelper::getValue($arContact, [
                        'PROPERTIES',
