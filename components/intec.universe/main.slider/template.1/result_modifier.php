...
foreach ($arResult['ITEMS'] as &$arItem) {
    ...
    if (!empty($arParams['PROPERTY_HEADER_OVER'])) {
        ...
        if (!empty($arProperty['VALUE'])) {
            ...
            $arItem['DATA']['HEADER']['OVER'] = Html::stripTags($arProperty, ['br']);
        }
    }
    ...
    if (!empty($arParams['PROPERTY_HEADER'])) {
        ...
        if (!empty($arProperty['VALUE'])) {
            ...
            $arItem['DATA']['HEADER']['VALUE'] = Html::stripTags($arProperty, ['br']);
        }
    }
    ...
    if (!empty($arParams['PROPERTY_PICTURE'])) {
        ...
        if (!empty($arProperty['VALUE'])) {
            ...
            if (!empty($arProperty['FILE_VALUE'])) {
                $arItem['DATA']['PICTURE'] = $arProperty['FILE_VALUE'];
                $arItem['DATA']['PICTURE']['ALING'] = [
                    'VERTICAL' => 'center'
                ];
            }
        }

        if (!empty($arItem['DATA']['PICTURE']) && !empty($arParams['PROPERTY_PICTURE_ALIGN_VERTICAL'])) {
            $arProperty = ArrayHelper::getValue($arItem, [
                'PROPERTIES',
            ...
            if (!empty($arProperty)) {
                ...
                $arProperty = StringHelper::replace($arProperty, [
                    'top' => 'start',
                    'center' => 'center',
                    'bottom' => 'end'
                ]);
            ...
            $arProperty = ArrayHelper::fromRange([
                'center',
                'end',
                'start'
            ], $arProperty);

            $arItem['DATA']['PICTURE']['ALIGN']['VERTICAL'] = $arProperty;
        }
    }
    ...
    if (!empty($arParams['PROPERTY_ADDITIONAL'])) {
        ...
        if (!empty($arProperty['VALUE'])) {
            ...
            if (!empty($arProperty['DISPLAY_VALUE']) && Type::isArray($arProperty['DISPLAY_VALUE'])) {
