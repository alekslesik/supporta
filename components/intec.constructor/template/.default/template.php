 ...
 * @return bool
 */
$draw = function ($container) use (&$draw, &$flag, &$isHeader, &$isFooter, &$isRender, &$storageArea, &$pagesMainBlocks, &$footerBlocks, &$USER, &$arResult) {
            ...
            if ($storageArea->code === 'header') {
                switch ($container->code) {
                    case 'basket-fixed': {
                        ...
                        $show = $show && ArrayHelper::isIn(Properties::get('header-template'), [2, 3, 7, 8, 9, 10, 12, 13, 14, 15, 16]);
                        $container->display = $show;
                        $variator = $container->getVariator(true);

                        if ($show && !empty($variator)) {
                ...
                if (StringHelper::startsWith($container->code, 'footer-blocks.')) {
                    ...
                    if (!empty($block)) {
                        ...
                        if (!empty($variator)) {
                            ...
                            foreach ($variants as $variant) {
                                if ($variant->code == $block['template'])
                                    break;
