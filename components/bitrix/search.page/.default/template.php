                ...
                <table class="search-form">
                    <tbody><tr>
                        <td class="search-form-input">
                            ...
                            <?else:?>
                                <input class="search-query intec-ui intec-ui-control-input intec-ui-mod-block intec-ui-mod-round-2 intec-ui-size-1" type="text" placeholder="<?=GetMessage('CT_PLACEHOLDER')?>" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" />
                            <?endif;?>
                        </td>
                        <td  class="search-form-button">
                            <button class="search-button search-form-button intec-ui intec-ui-control-button intec-ui-mod-round-2 intec-ui-scheme-current intec-ui-size-1" type="submit" value="<?echo GetMessage("CT_BSP_GO")?>">
                ...
                <div class="search-advanced">
                    <div class="search-advanced-result">
                        <?if(is_object($arResult["NAV_RESULT"])):?>
                            <div class="search-result"><?=GetMessage("CT_BSP_FOUND")?>: <?=$arResult["NAV_RESULT"]->SelectedRowsCount()?></div>
                        <?endif;?>
                        <?
                        $arWhere = array();
                    ...
                    <?if($arParams["SHOW_WHERE"] || $arParams["SHOW_WHEN"]):?>
                        ...
                        <div id="search_params" class="search-filter" style="display:<?echo $arResult["REQUEST"]["FROM"] || $arResult["REQUEST"]["TO"] || $arResult["REQUEST"]["WHERE"]? 'block': 'none'?>">
                            ...
                            <table class="search-filter" cellspacing="0"><tbody>
                                <?if($arParams["SHOW_WHERE"]):?>
                                <tr>
                                    <td class="search-filter-name"><?=GetMessage("CT_BSP_WHERE")?></td>
                                ...
                                <?if($arParams["SHOW_WHEN"]):?>
                                <tr>
                                    <td class="search-filter-name"><?=GetMessage("CT_BSP_WHEN")?></td>
                                    <td class="search-filter-field">
                                        <?$APPLICATION->IncludeComponent(
                    ...
                    <?foreach($arResult["SEARCH"] as $arItem):?>
                        <div class="search-item">
                            <?if (!empty($arItem['PICTURE'])) {?>
                                <a href="<?=$arItem["URL"]?>" class="search-item-img">
                            ...
                            <h4 class="search-item-name-h4">
                                <a href="<?=$arItem["URL"]?>" class="search-item-name intec-cl-text"><?=$arItem["TITLE_FORMATED"]?></a>
                            </h4>
                            ...
                            <div class="search-item-meta">
                                ...
                                <?if($arParams["SHOW_ITEM_TAGS"] != "N" && !empty($arItem["TAGS"])):?>
                                    <div class="search-item-tags"><label><?=GetMessage("CT_BSP_ITEM_TAGS")?>: </label><?
                                ...
                                <?if($arParams["SHOW_ITEM_DATE_CHANGE"] != "N"):?>
                                    <div class="search-item-date"><label><?=GetMessage("CT_BSP_DATE_CHANGE")?>: </label><span><?echo $arItem["DATE_CHANGE"]?></span></div>
                                <?endif;?>
                            </div>
                            <?endif?>
