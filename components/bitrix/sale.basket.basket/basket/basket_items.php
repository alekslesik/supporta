...
$arFastOrder = [];
$arFastOrder['USE'] = $arParams['FAST_ORDER_USE'] === 'Y';
...
if ($normalCount > 0) { ?>
	<div id="basket_items_list">
					...
					<tr id="<?= $arItem['ID'] ?>">
							...
							if ($arHeader['id'] == 'NAME') {
								...
								<td class="item">
									<h2 class="bx_ordercart_itemtitle">
                                        <?= Html::beginTag('a', [
                                            'href' => !empty($arItem['DETAIL_PAGE_URL']) ? $arItem['DETAIL_PAGE_URL'] : null
                                        ]) ?>
                                            <?= $arItem['NAME'] ?>
                                        <?= Html::endTag('a') ?>
									</h2>
						...
						if ($bDelayColumn || $bDeleteColumn) { ?>
							<td class="control">
								...
								if ($bDeleteColumn) { ?>
									<a class="delete-item far fa-times"
		...
		<div class="bx_ordercart_order_pay">
			<div class="row">
				<div class="col-xs-12 col-sm-6 bx_ordercart_coupon_column" id="coupons_block">
                    <?php if ($arParams['HIDE_COUPON'] != 'Y') { ?>
                        <div class="intec-ordercart_coupon">
                            <span class="intec-ordercart-coupon_name">
                                <?= Loc::getMessage('STB_COUPON_PROMT') ?>
                            </span>
                            <input type="text" class="intec-input" id="coupon" name="COUPON" value="" onchange="enterCoupon();">
                        </div>
                        <?php if (!empty($arResult['COUPON_LIST'])) { ?>
                            <?php foreach ($arResult['COUPON_LIST'] as $arCoupon) { ?>
                            <?php
                                $sClass = 'disabled';
                            ...
                            ?>
                                <div class="bx_ordercart_coupon"><?php
                                    echo Html::textInput('OLD_COUPON[]', $arCoupon['COUPON'], [
                                        'class' => [
                                            'intec-input',
                                            $sClass
                                        ],
                                    ...
                                    if (isset($arCoupon['CHECK_CODE_TEXT'])) {
                                        echo is_array($arCoupon['CHECK_CODE_TEXT']) ?
                                            implode('<br>', $arCoupon['CHECK_CODE_TEXT']) :
                                            $arCoupon['CHECK_CODE_TEXT'];
                                    }
