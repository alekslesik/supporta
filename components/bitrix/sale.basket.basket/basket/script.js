		...
		'div',
		{
			...
			children: [
				BX.create(
					...
					{
						props: {
							className: 'intec-input ' + couponClass,
							type: 'text',
			...
			if (!!couponsCollection)
			{
				...
				for (i = 0; i < res.COUPON_LIST.length; i++)
				{
					...
					if (couponFound)
					{
						...
						BX.adjust(couponsCollection[key], {props: {className: 'intec-input ' + couponClass}});
...
BX.ready(function() {
	...
	$('#basket_form .jsClearBasket').on('click', function(){
		var $currentTab = $('#basket_form .bx_sort_container.tabs a.tab.current'),
			data = {};

		switch ($currentTab.attr('id')) {
			case 'basket_toolbar_button':
				data.basket = 'Y';
				break;
			case 'basket_toolbar_button_delayed':
				data.delay = 'Y';
				break;
		}

		universe.basket.clear(data, function(){
			location.reload();
		});
	});
});